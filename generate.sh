...
if $RESOURCE_GROUP_NEEDED
then
    ## resource group
    echo "Creating Resource Group....."
    rg_name=$(\
        az group create \
        --name rg-$project_name_formatted-$TARGET_ENV-$region_formatted \
...
if $FUNCTION_APP_NEEDED
then
    ## storage account
    echo "Creating Storage Account....."
    st_name=$(\
        az storage account create \
        --name st${project_name_formatt// /}$TARGET_ENV${region_formatted// /}001 \
    ...
    func_name=$(\
        ...
        --name func-$project_name_formatted-$TARGET_ENV-$region_formatted \
        --resource-group $rg_name \
...
then
    ...
    sqls_name=$(\
        ...
        --name sqls-$project_name_formatted-$TARGET_ENV-$region_formatted \
        --resource-group $rg_name \
        --location $region_formatted \
        --admin-user sqls-$project_name_formatted-$TARGET_ENV-$region_formatted-admin \
    ...
    sqldb_name=$(\
        az sql db create \
        --name sqldb-$project_name_formatted-$TARGET_ENV-$region_formatted \
        --resource-group $rg_name \
        --server $sqls_name \
    ...
    then
        ...
        sqldb_shadow_name=$(\
            az sql db create \
            --name sqldb-$project_name_formatted-$TARGET_ENV-$region_formatted-shadow \
            --resource-group $rg_name \
            --server $sqls_name \
            --service-objective Basic \
            --max-size 500MB \
